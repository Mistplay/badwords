...
var Filter = (function() {
  ...
  function Filter(options) {
    ...
    this.badWordsList = options.reallyBadWords ? options.reallyBadWords.concat(this.list) : this.list;
  }
  ...
  Filter.prototype.isProfaneLike = function profaneLike(word) {
    return !!~this.exclude.indexOf(word) ? false : (!!~this.list.indexOf(word) ||
      this.badWordsList.map(w => new RegExp(w.replace(/(\W)/g, '\\$1'), 'gi').test(word)).includes(true));
  };
